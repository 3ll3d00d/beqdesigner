...
> Status of what this file describes: chunks 19-25 and 26a built (M0-M2, and the read-only work list), 26b-28 not started; the status of each chunk lives in the index
...
- **Done (2026-09-19); how it differs from the list above. Read-only as specified: the window runs, accepts, publishes and commits nothing.** The index schema is unchanged; no `pipeline/` code changed.
  - **Files.** `model/worklist.py` (`WorkListWindow`, the `_ScanJob` `QRunnable`/`_ScanSignals`, `describe_sources()`, `format_time()`), `model/worklist_model.py` (`WorkListModel`, `WorkListProxy`, chip and search rules, roles), `model/worklist_profile.py`
    ...
    open), and two preferences: `LIBRARY_PROFILE_PATH` (default empty = bootstrap) and `WORKLIST_GEOMETRY`. `LibrarySyncDialog` is untouched. The window's empty-state button emits `settings_requested`, which `app.py` wires to the Library Sync dialog (where the setup lives until 26c).
...
- **Done (2026-09-19); how it differs from the list above. This completes M3: the GUI shows what needs doing, does the machine work, and publishes and commits from a selection.** No `pipeline/` code changed and the index schema is unchanged. It **accepts nothing** (bulk accept UI is 27c; the threshold preference is still not added).
  ...
  - **Not done / open.** No per-item drill-in (27); per-title "Ignore titles like this" (26c); no per-source Rescan; no "extract only" button; the queue entries' *accept* is not here; a profile file's `designers:` section is not registered by the window (the CLI does that), so a file profile naming an unregistered designer shows the setup problem; a title selected in a view that no longer lists it (after it moved chip) drops out of the selection; the run panel keeps the last outcome until the next action.
