...
class GraphRenderer:
        ...
        for f in self.__graph.filters:
            if isinstance(f, Sequence):
                if isinstance(f, CompoundRoutingFilter):
                    ranks.append([])
                for f1 in f:
                    added_nodes = self.__process_filter(f1, last_node_by_channel, nodes, edges)
                    if isinstance(f1, XOFilter):
                        ranks[-1].extend(added_nodes)
            else:
                self.__process_filter(f, last_node_by_channel, nodes, edges)
        ...
        if isinstance(f, Mix):
            ...
            if f.mix_type == MixType.ADD or f.mix_type == MixType.SUBTRACT:
                ...
                if not last_dst or not re.search(fr'.*\[label=".*{target_name}"].*', nodes[last_dst],
                                                 flags=re.DOTALL):
                    added_nodes.append(self.__create_node(dst_channel, f, nodes, suffix=target_name))
                ...
                added_nodes.append(self.__create_node(dst_channel, f, nodes))
                last_node_by_channel[dst_channel] = added_nodes[-1]
                if last_src_node:
                ...
                src_node_name = self.__create_node(src_channel, f, nodes)
                dst_node_name = self.__create_node(dst_channel, f, nodes)
                previous_src_node = last_node_by_channel[src_channel]
                previous_dst_node = last_node_by_channel[dst_channel]
                if previous_src_node:
                    self.__add_edge(edges, previous_src_node, dst_node_name)
                if previous_dst_node:
                    self.__add_edge(edges, previous_dst_node, src_node_name)
            ...
            for c in channel_names:
                added_nodes.append(self.__create_node(c, f, nodes))
    ...
    def __create_node(self, channel: str, f: Filter, node_cache: Dict[str, str], suffix: str = None) -> str:
