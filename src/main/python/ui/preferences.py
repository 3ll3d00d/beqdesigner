# Form implementation generated from reading ui file 'preferences.ui'
#
# Created by: PyQt6 UI code generator 6.11.0
#
# WARNING: Any manual changes made to this file will be lost when pyuic6 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt6 import QtCore, QtGui, QtWidgets


class Ui_preferencesDialog(object):
    def setupUi(self, preferencesDialog):
        preferencesDialog.setObjectName("preferencesDialog")
        preferencesDialog.resize(375, 583)
        self.verticalLayout = QtWidgets.QVBoxLayout(preferencesDialog)
        self.verticalLayout.setObjectName("verticalLayout")
        self.panes = QtWidgets.QVBoxLayout()
        self.panes.setObjectName("panes")
        self.toolBox = QtWidgets.QToolBox(parent=preferencesDialog)
        self.toolBox.setObjectName("toolBox")
        self.binariesPage = QtWidgets.QWidget()
        self.binariesPage.setGeometry(QtCore.QRect(0, 0, 361, 273))
        self.binariesPage.setObjectName("binariesPage")
        self.binariesPane = QtWidgets.QGridLayout(self.binariesPage)
        self.binariesPane.setObjectName("binariesPane")
        self.minidspRsPathPicker = QtWidgets.QToolButton(parent=self.binariesPage)
        self.minidspRsPathPicker.setObjectName("minidspRsPathPicker")
        self.binariesPane.addWidget(self.minidspRsPathPicker, 2, 2, 1, 1)
        self.ffprobeDirectoryPicker = QtWidgets.QToolButton(parent=self.binariesPage)
        self.ffprobeDirectoryPicker.setObjectName("ffprobeDirectoryPicker")
        self.binariesPane.addWidget(self.ffprobeDirectoryPicker, 1, 2, 1, 1)
        self.ffmpegLabel = QtWidgets.QLabel(parent=self.binariesPage)
        self.ffmpegLabel.setObjectName("ffmpegLabel")
        self.binariesPane.addWidget(self.ffmpegLabel, 0, 0, 1, 1)
        self.ffprobeDirectory = QtWidgets.QLineEdit(parent=self.binariesPage)
        self.ffprobeDirectory.setEnabled(False)
        self.ffprobeDirectory.setObjectName("ffprobeDirectory")
        self.binariesPane.addWidget(self.ffprobeDirectory, 1, 1, 1, 1)
        self.ffmpegDirectoryPicker = QtWidgets.QToolButton(parent=self.binariesPage)
        self.ffmpegDirectoryPicker.setObjectName("ffmpegDirectoryPicker")
        self.binariesPane.addWidget(self.ffmpegDirectoryPicker, 0, 2, 1, 1)
        self.ffmpegDirectory = QtWidgets.QLineEdit(parent=self.binariesPage)
        self.ffmpegDirectory.setEnabled(False)
        self.ffmpegDirectory.setObjectName("ffmpegDirectory")
        self.binariesPane.addWidget(self.ffmpegDirectory, 0, 1, 1, 1)
        self.ffprobeLabel = QtWidgets.QLabel(parent=self.binariesPage)
        self.ffprobeLabel.setObjectName("ffprobeLabel")
        self.binariesPane.addWidget(self.ffprobeLabel, 1, 0, 1, 1)
        self.minidspRsPath = QtWidgets.QLineEdit(parent=self.binariesPage)
        self.minidspRsPath.setEnabled(False)
        self.minidspRsPath.setReadOnly(True)
        self.minidspRsPath.setObjectName("minidspRsPath")
        self.binariesPane.addWidget(self.minidspRsPath, 2, 1, 1, 1)
        self.minidspRsPathLabel = QtWidgets.QLabel(parent=self.binariesPage)
        self.minidspRsPathLabel.setObjectName("minidspRsPathLabel")
        self.binariesPane.addWidget(self.minidspRsPathLabel, 2, 0, 1, 1)
        self.minidspRsOptionsLabel = QtWidgets.QLabel(parent=self.binariesPage)
        self.minidspRsOptionsLabel.setObjectName("minidspRsOptionsLabel")
        self.binariesPane.addWidget(self.minidspRsOptionsLabel, 3, 0, 1, 1)
        self.minidspRsOptions = QtWidgets.QLineEdit(parent=self.binariesPage)
        self.minidspRsOptions.setObjectName("minidspRsOptions")
        self.binariesPane.addWidget(self.minidspRsOptions, 3, 1, 1, 1)
        self.toolBox.addItem(self.binariesPage, "")
        self.analysisPage = QtWidgets.QWidget()
        self.analysisPage.setGeometry(QtCore.QRect(0, 0, 361, 273))
        self.analysisPage.setObjectName("analysisPage")
        self.analysisPane = QtWidgets.QGridLayout(self.analysisPage)
        self.analysisPane.setObjectName("analysisPane")
        self.peakAnalysisWindow = QtWidgets.QComboBox(parent=self.analysisPage)
        self.peakAnalysisWindow.setObjectName("peakAnalysisWindow")
        self.analysisPane.addWidget(self.peakAnalysisWindow, 3, 1, 1, 1)
        self.peakAnalysisWindowLabel = QtWidgets.QLabel(parent=self.analysisPage)
        self.peakAnalysisWindowLabel.setObjectName("peakAnalysisWindowLabel")
        self.analysisPane.addWidget(self.peakAnalysisWindowLabel, 3, 0, 1, 1)
        self.avgAnalysisWindowLabel = QtWidgets.QLabel(parent=self.analysisPage)
        self.avgAnalysisWindowLabel.setObjectName("avgAnalysisWindowLabel")
        self.analysisPane.addWidget(self.avgAnalysisWindowLabel, 2, 0, 1, 1)
        self.avgAnalysisWindow = QtWidgets.QComboBox(parent=self.analysisPage)
        self.avgAnalysisWindow.setObjectName("avgAnalysisWindow")
        self.analysisPane.addWidget(self.avgAnalysisWindow, 2, 1, 1, 1)
        self.targetFsLabel = QtWidgets.QLabel(parent=self.analysisPage)
        self.targetFsLabel.setObjectName("targetFsLabel")
        self.analysisPane.addWidget(self.targetFsLabel, 0, 0, 1, 1)
        self.targetFs = QtWidgets.QComboBox(parent=self.analysisPage)
        self.targetFs.setObjectName("targetFs")
        self.targetFs.addItem("")
        self.targetFs.addItem("")
        self.targetFs.addItem("")
        self.targetFs.addItem("")
        self.targetFs.addItem("")
        self.targetFs.addItem("")
        self.analysisPane.addWidget(self.targetFs, 0, 1, 1, 1)
        self.resolutionLabel = QtWidgets.QLabel(parent=self.analysisPage)
        self.resolutionLabel.setObjectName("resolutionLabel")
        self.analysisPane.addWidget(self.resolutionLabel, 1, 0, 1, 1)
        self.resolutionSelect = QtWidgets.QComboBox(parent=self.analysisPage)
        self.resolutionSelect.setObjectName("resolutionSelect")
        self.resolutionSelect.addItem("")
        self.resolutionSelect.addItem("")
        self.resolutionSelect.addItem("")
        self.resolutionSelect.addItem("")
        self.resolutionSelect.addItem("")
        self.analysisPane.addWidget(self.resolutionSelect, 1, 1, 1, 1)
        self.analysisPane.setColumnStretch(1, 1)
        self.toolBox.addItem(self.analysisPage, "")
        self.extractionPage = QtWidgets.QWidget()
        self.extractionPage.setGeometry(QtCore.QRect(0, 0, 361, 273))
        self.extractionPage.setObjectName("extractionPage")
        self.extractPane = QtWidgets.QGridLayout(self.extractionPage)
        self.extractPane.setObjectName("extractPane")
        self.defaultOutputDirectoryLabel = QtWidgets.QLabel(parent=self.extractionPage)
        self.defaultOutputDirectoryLabel.setObjectName("defaultOutputDirectoryLabel")
        self.extractPane.addWidget(self.defaultOutputDirectoryLabel, 0, 0, 1, 1)
        self.defaultOutputDirectory = QtWidgets.QLineEdit(parent=self.extractionPage)
        self.defaultOutputDirectory.setEnabled(False)
        self.defaultOutputDirectory.setObjectName("defaultOutputDirectory")
        self.extractPane.addWidget(self.defaultOutputDirectory, 0, 1, 1, 1)
        self.defaultOutputDirectoryPicker = QtWidgets.QToolButton(parent=self.extractionPage)
        self.defaultOutputDirectoryPicker.setObjectName("defaultOutputDirectoryPicker")
        self.extractPane.addWidget(self.defaultOutputDirectoryPicker, 0, 2, 1, 1)
        self.extractCompleteAudioFileLabel = QtWidgets.QLabel(parent=self.extractionPage)
        self.extractCompleteAudioFileLabel.setObjectName("extractCompleteAudioFileLabel")
        self.extractPane.addWidget(self.extractCompleteAudioFileLabel, 1, 0, 1, 1)
        self.extractCompleteAudioFile = QtWidgets.QLineEdit(parent=self.extractionPage)
        self.extractCompleteAudioFile.setEnabled(False)
        self.extractCompleteAudioFile.setObjectName("extractCompleteAudioFile")
        self.extractPane.addWidget(self.extractCompleteAudioFile, 1, 1, 1, 1)
        self.extractCompleteAudioFilePicker = QtWidgets.QToolButton(parent=self.extractionPage)
        self.extractCompleteAudioFilePicker.setObjectName("extractCompleteAudioFilePicker")
        self.extractPane.addWidget(self.extractCompleteAudioFilePicker, 1, 2, 1, 1)
        self.extractOptions1 = QtWidgets.QHBoxLayout()
        self.extractOptions1.setObjectName("extractOptions1")
        self.includeOriginal = QtWidgets.QCheckBox(parent=self.extractionPage)
        self.includeOriginal.setObjectName("includeOriginal")
        self.extractOptions1.addWidget(self.includeOriginal)
        self.compress = QtWidgets.QCheckBox(parent=self.extractionPage)
        self.compress.setObjectName("compress")
        self.extractOptions1.addWidget(self.compress)
        self.extractPane.addLayout(self.extractOptions1, 2, 0, 1, 3)
        self.extractOptions2 = QtWidgets.QHBoxLayout()
        self.extractOptions2.setObjectName("extractOptions2")
        self.monoMix = QtWidgets.QCheckBox(parent=self.extractionPage)
        self.monoMix.setObjectName("monoMix")
        self.extractOptions2.addWidget(self.monoMix)
        self.decimate = QtWidgets.QCheckBox(parent=self.extractionPage)
        self.decimate.setObjectName("decimate")
        self.extractOptions2.addWidget(self.decimate)
        self.includeSubtitles = QtWidgets.QCheckBox(parent=self.extractionPage)
        self.includeSubtitles.setObjectName("includeSubtitles")
        self.extractOptions2.addWidget(self.includeSubtitles)
        self.extractPane.addLayout(self.extractOptions2, 3, 0, 1, 3)
        self.toolBox.addItem(self.extractionPage, "")
        self.designerPage = QtWidgets.QWidget()
        self.designerPage.setGeometry(QtCore.QRect(0, 0, 361, 273))
        self.designerPage.setObjectName("designerPage")
        self.designerPane = QtWidgets.QVBoxLayout(self.designerPage)
        self.designerPane.setObjectName("designerPane")
        self.designersHelpLabel = QtWidgets.QLabel(parent=self.designerPage)
        self.designersHelpLabel.setWordWrap(True)
        self.designersHelpLabel.setObjectName("designersHelpLabel")
        self.designerPane.addWidget(self.designersHelpLabel)
        self.designersTable = QtWidgets.QTableWidget(parent=self.designerPage)
        self.designersTable.setColumnCount(3)
        self.designersTable.setObjectName("designersTable")
        self.designersTable.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.designersTable.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.designersTable.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.designersTable.setHorizontalHeaderItem(2, item)
        self.designerPane.addWidget(self.designersTable)
        self.designerRowButtonsLayout = QtWidgets.QHBoxLayout()
        self.designerRowButtonsLayout.setObjectName("designerRowButtonsLayout")
        self.designersAddButton = QtWidgets.QPushButton(parent=self.designerPage)
        self.designersAddButton.setObjectName("designersAddButton")
        self.designerRowButtonsLayout.addWidget(self.designersAddButton)
        self.designersRemoveButton = QtWidgets.QPushButton(parent=self.designerPage)
        self.designersRemoveButton.setObjectName("designersRemoveButton")
        self.designerRowButtonsLayout.addWidget(self.designersRemoveButton)
        self.designerPane.addLayout(self.designerRowButtonsLayout)
        self.designQueueDirPane = QtWidgets.QGridLayout()
        self.designQueueDirPane.setObjectName("designQueueDirPane")
        self.designQueueDirLabel = QtWidgets.QLabel(parent=self.designerPage)
        self.designQueueDirLabel.setObjectName("designQueueDirLabel")
        self.designQueueDirPane.addWidget(self.designQueueDirLabel, 0, 0, 1, 1)
        self.designQueueDir = QtWidgets.QLineEdit(parent=self.designerPage)
        self.designQueueDir.setEnabled(False)
        self.designQueueDir.setObjectName("designQueueDir")
        self.designQueueDirPane.addWidget(self.designQueueDir, 0, 1, 1, 1)
        self.designQueueDirPicker = QtWidgets.QToolButton(parent=self.designerPage)
        self.designQueueDirPicker.setObjectName("designQueueDirPicker")
        self.designQueueDirPane.addWidget(self.designQueueDirPicker, 0, 2, 1, 1)
        self.defaultDesignerLabel = QtWidgets.QLabel(parent=self.designerPage)
        self.defaultDesignerLabel.setObjectName("defaultDesignerLabel")
        self.designQueueDirPane.addWidget(self.defaultDesignerLabel, 1, 0, 1, 1)
        self.defaultDesignerCombo = QtWidgets.QComboBox(parent=self.designerPage)
        self.defaultDesignerCombo.setObjectName("defaultDesignerCombo")
        self.designQueueDirPane.addWidget(self.defaultDesignerCombo, 1, 1, 1, 2)
        self.designerPane.addLayout(self.designQueueDirPane)
        self.toolBox.addItem(self.designerPage, "")
        self.jriverPage = QtWidgets.QWidget()
        self.jriverPage.setGeometry(QtCore.QRect(0, 0, 361, 273))
        self.jriverPage.setObjectName("jriverPage")
        self.jriverPane = QtWidgets.QVBoxLayout(self.jriverPage)
        self.jriverPane.setObjectName("jriverPane")
        self.jriverHelpLabel = QtWidgets.QLabel(parent=self.jriverPage)
        self.jriverHelpLabel.setWordWrap(True)
        self.jriverHelpLabel.setObjectName("jriverHelpLabel")
        self.jriverPane.addWidget(self.jriverHelpLabel)
        self.toolBox.addItem(self.jriverPage, "")
        self.stylePage = QtWidgets.QWidget()
        self.stylePage.setGeometry(QtCore.QRect(0, 0, 361, 273))
        self.stylePage.setObjectName("stylePage")
        self.stylePane = QtWidgets.QGridLayout(self.stylePage)
        self.stylePane.setObjectName("stylePane")
        self.smoothGraphs = QtWidgets.QCheckBox(parent=self.stylePage)
        self.smoothGraphs.setObjectName("smoothGraphs")
        self.stylePane.addWidget(self.smoothGraphs, 2, 1, 1, 1)
        self.themePicker = QtWidgets.QComboBox(parent=self.stylePage)
        self.themePicker.setObjectName("themePicker")
        self.themePicker.addItem("")
        self.stylePane.addWidget(self.themePicker, 0, 1, 1, 1)
        self.themeLabel = QtWidgets.QLabel(parent=self.stylePage)
        self.themeLabel.setObjectName("themeLabel")
        self.stylePane.addWidget(self.themeLabel, 0, 0, 1, 1)
        self.speclabLineStyle = QtWidgets.QCheckBox(parent=self.stylePage)
        self.speclabLineStyle.setObjectName("speclabLineStyle")
        self.stylePane.addWidget(self.speclabLineStyle, 1, 1, 1, 1)
        self.imageFormat = QtWidgets.QComboBox(parent=self.stylePage)
        self.imageFormat.setObjectName("imageFormat")
        self.imageFormat.addItem("")
        self.imageFormat.addItem("")
        self.stylePane.addWidget(self.imageFormat, 3, 1, 1, 1)
        self.imageFormatLabel = QtWidgets.QLabel(parent=self.stylePage)
        self.imageFormatLabel.setObjectName("imageFormatLabel")
        self.stylePane.addWidget(self.imageFormatLabel, 3, 0, 1, 1)
        self.stylePane.setColumnStretch(1, 1)
        self.toolBox.addItem(self.stylePage, "")
        self.graphPage = QtWidgets.QWidget()
        self.graphPage.setGeometry(QtCore.QRect(0, 0, 361, 273))
        self.graphPage.setObjectName("graphPage")
        self.graphPane = QtWidgets.QVBoxLayout(self.graphPage)
        self.graphPane.setObjectName("graphPane")
        self.freqIsLogScale = QtWidgets.QCheckBox(parent=self.graphPage)
        self.freqIsLogScale.setChecked(True)
        self.freqIsLogScale.setObjectName("freqIsLogScale")
        self.graphPane.addWidget(self.freqIsLogScale)
        self.precalcSmoothing = QtWidgets.QCheckBox(parent=self.graphPage)
        self.precalcSmoothing.setChecked(True)
        self.precalcSmoothing.setObjectName("precalcSmoothing")
        self.graphPane.addWidget(self.precalcSmoothing)
        self.expandYLimits = QtWidgets.QCheckBox(parent=self.graphPage)
        self.expandYLimits.setObjectName("expandYLimits")
        self.graphPane.addWidget(self.expandYLimits)
        self.xminmaxLayout = QtWidgets.QHBoxLayout()
        self.xminmaxLayout.setObjectName("xminmaxLayout")
        self.xminmaxLabel = QtWidgets.QLabel(parent=self.graphPage)
        self.xminmaxLabel.setObjectName("xminmaxLabel")
        self.xminmaxLayout.addWidget(self.xminmaxLabel)
        self.xmin = QtWidgets.QSpinBox(parent=self.graphPage)
        self.xmin.setMinimum(1)
        self.xmin.setMaximum(23999)
        self.xmin.setProperty("value", 2)
        self.xmin.setObjectName("xmin")
        self.xminmaxLayout.addWidget(self.xmin)
        self.xmax = QtWidgets.QSpinBox(parent=self.graphPage)
        self.xmax.setMinimum(2)
        self.xmax.setMaximum(24000)
        self.xmax.setProperty("value", 160)
        self.xmax.setObjectName("xmax")
        self.xminmaxLayout.addWidget(self.xmax)
        self.graphPane.addLayout(self.xminmaxLayout)
        self.toolBox.addItem(self.graphPage, "")
        self.filterPage = QtWidgets.QWidget()
        self.filterPage.setGeometry(QtCore.QRect(0, 0, 361, 273))
        self.filterPage.setObjectName("filterPage")
        self.filterPane = QtWidgets.QGridLayout(self.filterPage)
        self.filterPane.setObjectName("filterPane")
        self.hsFreqLabel = QtWidgets.QLabel(parent=self.filterPage)
        self.hsFreqLabel.setObjectName("hsFreqLabel")
        self.filterPane.addWidget(self.hsFreqLabel, 3, 0, 1, 1)
        self.lsFreqLabel = QtWidgets.QLabel(parent=self.filterPage)
        self.lsFreqLabel.setObjectName("lsFreqLabel")
        self.filterPane.addWidget(self.lsFreqLabel, 1, 0, 1, 1)
        self.bmlpfLabel = QtWidgets.QLabel(parent=self.filterPage)
        self.bmlpfLabel.setObjectName("bmlpfLabel")
        self.filterPane.addWidget(self.bmlpfLabel, 6, 0, 1, 1)
        self.peakQLabel = QtWidgets.QLabel(parent=self.filterPage)
        self.peakQLabel.setObjectName("peakQLabel")
        self.filterPane.addWidget(self.peakQLabel, 4, 0, 1, 1)
        self.lsQLabel = QtWidgets.QLabel(parent=self.filterPage)
        self.lsQLabel.setObjectName("lsQLabel")
        self.filterPane.addWidget(self.lsQLabel, 0, 0, 1, 1)
        self.hsQLabel = QtWidgets.QLabel(parent=self.filterPage)
        self.hsQLabel.setObjectName("hsQLabel")
        self.filterPane.addWidget(self.hsQLabel, 2, 0, 1, 1)
        self.bmlpfFreq = QtWidgets.QSpinBox(parent=self.filterPage)
        self.bmlpfFreq.setMinimum(20)
        self.bmlpfFreq.setMaximum(160)
        self.bmlpfFreq.setProperty("value", 80)
        self.bmlpfFreq.setObjectName("bmlpfFreq")
        self.filterPane.addWidget(self.bmlpfFreq, 6, 1, 1, 1)
        self.lsQ = QtWidgets.QDoubleSpinBox(parent=self.filterPage)
        self.lsQ.setDecimals(3)
        self.lsQ.setMinimum(0.001)
        self.lsQ.setMaximum(10.0)
        self.lsQ.setSingleStep(0.001)
        self.lsQ.setProperty("value", 0.707)
        self.lsQ.setObjectName("lsQ")
        self.filterPane.addWidget(self.lsQ, 0, 1, 1, 1)
        self.lsFreq = QtWidgets.QSpinBox(parent=self.filterPage)
        self.lsFreq.setMinimum(1)
        self.lsFreq.setMaximum(24000)
        self.lsFreq.setProperty("value", 20)
        self.lsFreq.setObjectName("lsFreq")
        self.filterPane.addWidget(self.lsFreq, 1, 1, 1, 1)
        self.peakFreqLabel = QtWidgets.QLabel(parent=self.filterPage)
        self.peakFreqLabel.setObjectName("peakFreqLabel")
        self.filterPane.addWidget(self.peakFreqLabel, 5, 0, 1, 1)
        self.hsQ = QtWidgets.QDoubleSpinBox(parent=self.filterPage)
        self.hsQ.setDecimals(3)
        self.hsQ.setMinimum(0.001)
        self.hsQ.setMaximum(10.0)
        self.hsQ.setSingleStep(0.001)
        self.hsQ.setProperty("value", 0.707)
        self.hsQ.setObjectName("hsQ")
        self.filterPane.addWidget(self.hsQ, 2, 1, 1, 1)
        self.hsFreq = QtWidgets.QSpinBox(parent=self.filterPage)
        self.hsFreq.setMinimum(1)
        self.hsFreq.setMaximum(24000)
        self.hsFreq.setProperty("value", 80)
        self.hsFreq.setObjectName("hsFreq")
        self.filterPane.addWidget(self.hsFreq, 3, 1, 1, 1)
        self.peakQ = QtWidgets.QDoubleSpinBox(parent=self.filterPage)
        self.peakQ.setDecimals(3)
        self.peakQ.setMinimum(0.001)
        self.peakQ.setMaximum(10.0)
        self.peakQ.setSingleStep(0.001)
        self.peakQ.setProperty("value", 1.0)
        self.peakQ.setObjectName("peakQ")
        self.filterPane.addWidget(self.peakQ, 4, 1, 1, 1)
        self.peakFreq = QtWidgets.QSpinBox(parent=self.filterPage)
        self.peakFreq.setMinimum(1)
        self.peakFreq.setMaximum(24000)
        self.peakFreq.setProperty("value", 20)
        self.peakFreq.setObjectName("peakFreq")
        self.filterPane.addWidget(self.peakFreq, 5, 1, 1, 1)
        self.filterPane.setColumnStretch(1, 1)
        self.toolBox.addItem(self.filterPage, "")
        self.systemPage = QtWidgets.QWidget()
        self.systemPage.setGeometry(QtCore.QRect(0, 0, 361, 273))
        self.systemPage.setObjectName("systemPage")
        self.systemPane = QtWidgets.QGridLayout(self.systemPage)
        self.systemPane.setObjectName("systemPane")
        self.checkForBetaUpdates = QtWidgets.QCheckBox(parent=self.systemPage)
        self.checkForBetaUpdates.setObjectName("checkForBetaUpdates")
        self.systemPane.addWidget(self.checkForBetaUpdates, 1, 0, 1, 1)
        self.checkForUpdates = QtWidgets.QCheckBox(parent=self.systemPage)
        self.checkForUpdates.setChecked(True)
        self.checkForUpdates.setObjectName("checkForUpdates")
        self.systemPane.addWidget(self.checkForUpdates, 0, 0, 1, 1)
        self.toolBox.addItem(self.systemPage, "")
        self.beqPage = QtWidgets.QWidget()
        self.beqPage.setGeometry(QtCore.QRect(0, 0, 361, 273))
        self.beqPage.setObjectName("beqPage")
        self.beqPane = QtWidgets.QGridLayout(self.beqPage)
        self.beqPane.setObjectName("beqPane")
        self.beqFiltersDir = QtWidgets.QLineEdit(parent=self.beqPage)
        self.beqFiltersDir.setReadOnly(True)
        self.beqFiltersDir.setObjectName("beqFiltersDir")
        self.beqPane.addWidget(self.beqFiltersDir, 0, 1, 1, 1)
        self.beqDirectoryPicker = QtWidgets.QToolButton(parent=self.beqPage)
        self.beqDirectoryPicker.setObjectName("beqDirectoryPicker")
        self.beqPane.addWidget(self.beqDirectoryPicker, 0, 2, 1, 1)
        self.beqDirectoryLabel = QtWidgets.QLabel(parent=self.beqPage)
        self.beqDirectoryLabel.setObjectName("beqDirectoryLabel")
        self.beqPane.addWidget(self.beqDirectoryLabel, 0, 0, 1, 1)
        self.toolBox.addItem(self.beqPage, "")
        self.panes.addWidget(self.toolBox)
        self.verticalLayout.addLayout(self.panes)
        self.buttonBox = QtWidgets.QDialogButtonBox(parent=preferencesDialog)
        self.buttonBox.setOrientation(QtCore.Qt.Orientation.Horizontal)
        self.buttonBox.setStandardButtons(QtWidgets.QDialogButtonBox.StandardButton.Cancel|QtWidgets.QDialogButtonBox.StandardButton.RestoreDefaults|QtWidgets.QDialogButtonBox.StandardButton.Save)
        self.buttonBox.setObjectName("buttonBox")
        self.verticalLayout.addWidget(self.buttonBox)

        self.retranslateUi(preferencesDialog)
        self.toolBox.setCurrentIndex(0)
        self.buttonBox.accepted.connect(preferencesDialog.accept) # type: ignore
        self.buttonBox.rejected.connect(preferencesDialog.reject) # type: ignore
        self.ffmpegDirectoryPicker.clicked.connect(preferencesDialog.showFfmpegDirectoryPicker) # type: ignore
        self.ffprobeDirectoryPicker.clicked.connect(preferencesDialog.showFfprobeDirectoryPicker) # type: ignore
        self.defaultOutputDirectoryPicker.clicked.connect(preferencesDialog.showDefaultOutputDirectoryPicker) # type: ignore
        self.extractCompleteAudioFilePicker.clicked.connect(preferencesDialog.showExtractCompleteSoundPicker) # type: ignore
        self.minidspRsPathPicker.clicked.connect(preferencesDialog.show_minidsp_rs_picker) # type: ignore
        self.designQueueDirPicker.clicked.connect(preferencesDialog.showDesignQueueDirPicker) # type: ignore
        QtCore.QMetaObject.connectSlotsByName(preferencesDialog)
        preferencesDialog.setTabOrder(self.ffmpegDirectory, self.ffmpegDirectoryPicker)
        preferencesDialog.setTabOrder(self.ffmpegDirectoryPicker, self.ffprobeDirectory)
        preferencesDialog.setTabOrder(self.ffprobeDirectory, self.ffprobeDirectoryPicker)
        preferencesDialog.setTabOrder(self.ffprobeDirectoryPicker, self.targetFs)
        preferencesDialog.setTabOrder(self.targetFs, self.resolutionSelect)
        preferencesDialog.setTabOrder(self.resolutionSelect, self.avgAnalysisWindow)
        preferencesDialog.setTabOrder(self.avgAnalysisWindow, self.peakAnalysisWindow)
        preferencesDialog.setTabOrder(self.peakAnalysisWindow, self.defaultOutputDirectory)
        preferencesDialog.setTabOrder(self.defaultOutputDirectory, self.defaultOutputDirectoryPicker)
        preferencesDialog.setTabOrder(self.defaultOutputDirectoryPicker, self.extractCompleteAudioFile)
        preferencesDialog.setTabOrder(self.extractCompleteAudioFile, self.extractCompleteAudioFilePicker)
        preferencesDialog.setTabOrder(self.extractCompleteAudioFilePicker, self.includeOriginal)
        preferencesDialog.setTabOrder(self.includeOriginal, self.compress)
        preferencesDialog.setTabOrder(self.compress, self.monoMix)
        preferencesDialog.setTabOrder(self.monoMix, self.decimate)
        preferencesDialog.setTabOrder(self.decimate, self.includeSubtitles)
        preferencesDialog.setTabOrder(self.includeSubtitles, self.themePicker)
        preferencesDialog.setTabOrder(self.themePicker, self.speclabLineStyle)
        preferencesDialog.setTabOrder(self.speclabLineStyle, self.smoothGraphs)
        preferencesDialog.setTabOrder(self.smoothGraphs, self.freqIsLogScale)
        preferencesDialog.setTabOrder(self.freqIsLogScale, self.precalcSmoothing)
        preferencesDialog.setTabOrder(self.precalcSmoothing, self.expandYLimits)
        preferencesDialog.setTabOrder(self.expandYLimits, self.xmin)
        preferencesDialog.setTabOrder(self.xmin, self.xmax)
        preferencesDialog.setTabOrder(self.xmax, self.lsQ)
        preferencesDialog.setTabOrder(self.lsQ, self.lsFreq)
        preferencesDialog.setTabOrder(self.lsFreq, self.hsQ)
        preferencesDialog.setTabOrder(self.hsQ, self.hsFreq)
        preferencesDialog.setTabOrder(self.hsFreq, self.peakQ)
        preferencesDialog.setTabOrder(self.peakQ, self.peakFreq)
        preferencesDialog.setTabOrder(self.peakFreq, self.bmlpfFreq)
        preferencesDialog.setTabOrder(self.bmlpfFreq, self.checkForUpdates)
        preferencesDialog.setTabOrder(self.checkForUpdates, self.checkForBetaUpdates)
        preferencesDialog.setTabOrder(self.checkForBetaUpdates, self.beqFiltersDir)
        preferencesDialog.setTabOrder(self.beqFiltersDir, self.beqDirectoryPicker)

    def retranslateUi(self, preferencesDialog):
        _translate = QtCore.QCoreApplication.translate
        preferencesDialog.setWindowTitle(_translate("preferencesDialog", "Preferences"))
        self.minidspRsPathPicker.setText(_translate("preferencesDialog", "..."))
        self.ffprobeDirectoryPicker.setText(_translate("preferencesDialog", "..."))
        self.ffmpegLabel.setText(_translate("preferencesDialog", "ffmpeg"))
        self.ffmpegDirectoryPicker.setText(_translate("preferencesDialog", "..."))
        self.ffprobeLabel.setText(_translate("preferencesDialog", "ffprobe"))
        self.minidspRsPathLabel.setText(_translate("preferencesDialog", "minidsp-rs"))
        self.minidspRsOptionsLabel.setText(_translate("preferencesDialog", "options"))
        self.toolBox.setItemText(self.toolBox.indexOf(self.binariesPage), _translate("preferencesDialog", "Binaries"))
        self.peakAnalysisWindowLabel.setText(_translate("preferencesDialog", "Peak Window"))
        self.avgAnalysisWindowLabel.setText(_translate("preferencesDialog", "Avg Window"))
        self.targetFsLabel.setText(_translate("preferencesDialog", "Target Fs"))
        self.targetFs.setItemText(0, _translate("preferencesDialog", "250 Hz"))
        self.targetFs.setItemText(1, _translate("preferencesDialog", "500 Hz"))
        self.targetFs.setItemText(2, _translate("preferencesDialog", "1000 Hz"))
        self.targetFs.setItemText(3, _translate("preferencesDialog", "2000 Hz"))
        self.targetFs.setItemText(4, _translate("preferencesDialog", "4000 Hz"))
        self.targetFs.setItemText(5, _translate("preferencesDialog", "8000 Hz"))
        self.resolutionLabel.setText(_translate("preferencesDialog", "Resolution"))
        self.resolutionSelect.setCurrentText(_translate("preferencesDialog", "0.25 Hz"))
        self.resolutionSelect.setItemText(0, _translate("preferencesDialog", "0.25 Hz"))
        self.resolutionSelect.setItemText(1, _translate("preferencesDialog", "0.5 Hz"))
        self.resolutionSelect.setItemText(2, _translate("preferencesDialog", "1.0 Hz"))
        self.resolutionSelect.setItemText(3, _translate("preferencesDialog", "2.0 Hz"))
        self.resolutionSelect.setItemText(4, _translate("preferencesDialog", "4.0 Hz"))
        self.toolBox.setItemText(self.toolBox.indexOf(self.analysisPage), _translate("preferencesDialog", "Analysis"))
        self.defaultOutputDirectoryLabel.setText(_translate("preferencesDialog", "Default Output Directory"))
        self.defaultOutputDirectoryPicker.setText(_translate("preferencesDialog", "..."))
        self.extractCompleteAudioFileLabel.setText(_translate("preferencesDialog", "Extract Complete Sound"))
        self.extractCompleteAudioFilePicker.setText(_translate("preferencesDialog", "..."))
        self.includeOriginal.setText(_translate("preferencesDialog", "Add Original Audio?"))
        self.compress.setText(_translate("preferencesDialog", "Compress Audio?"))
        self.monoMix.setText(_translate("preferencesDialog", "Mix to mono?"))
        self.decimate.setText(_translate("preferencesDialog", "Decimate?"))
        self.includeSubtitles.setText(_translate("preferencesDialog", "Add Subtitles?"))
        self.toolBox.setItemText(self.toolBox.indexOf(self.extractionPage), _translate("preferencesDialog", "Extraction"))
        self.designersHelpLabel.setText(_translate("preferencesDialog", "HTTP designer endpoints, available to Batch Design by name. Headers (optional) is a JSON object, e.g. {\"Authorization\": \"Bearer ...\"}."))
        item = self.designersTable.horizontalHeaderItem(0)
        item.setText(_translate("preferencesDialog", "Name"))
        item = self.designersTable.horizontalHeaderItem(1)
        item.setText(_translate("preferencesDialog", "URL"))
        item = self.designersTable.horizontalHeaderItem(2)
        item.setText(_translate("preferencesDialog", "Headers (JSON)"))
        self.designersAddButton.setText(_translate("preferencesDialog", "Add"))
        self.designersRemoveButton.setText(_translate("preferencesDialog", "Remove"))
        self.designQueueDirLabel.setText(_translate("preferencesDialog", "Review Queue Directory"))
        self.designQueueDirPicker.setText(_translate("preferencesDialog", "..."))
        self.defaultDesignerLabel.setText(_translate("preferencesDialog", "Default Designer"))
        self.toolBox.setItemText(self.toolBox.indexOf(self.designerPage), _translate("preferencesDialog", "Designers"))
        self.jriverHelpLabel.setText(_translate("preferencesDialog", "Media Center servers, shared by the JRiver filter manager and the Library Work List."))
        self.toolBox.setItemText(self.toolBox.indexOf(self.jriverPage), _translate("preferencesDialog", "JRiver"))
        self.smoothGraphs.setText(_translate("preferencesDialog", "Smooth?"))
        self.themePicker.setItemText(0, _translate("preferencesDialog", "default"))
        self.themeLabel.setText(_translate("preferencesDialog", "Theme"))
        self.speclabLineStyle.setText(_translate("preferencesDialog", "Speclab Line Colours?"))
        self.imageFormat.setItemText(0, _translate("preferencesDialog", "png"))
        self.imageFormat.setItemText(1, _translate("preferencesDialog", "jpg"))
        self.imageFormatLabel.setText(_translate("preferencesDialog", "Image Format"))
        self.toolBox.setItemText(self.toolBox.indexOf(self.stylePage), _translate("preferencesDialog", "Style"))
        self.freqIsLogScale.setText(_translate("preferencesDialog", "Frequency Axis Log Scale?"))
        self.precalcSmoothing.setText(_translate("preferencesDialog", "Precalculate Octave Smoothing?"))
        self.expandYLimits.setText(_translate("preferencesDialog", "Auto Expand Y Limits?"))
        self.xminmaxLabel.setText(_translate("preferencesDialog", "x min/max"))
        self.toolBox.setItemText(self.toolBox.indexOf(self.graphPage), _translate("preferencesDialog", "Graph"))
        self.hsFreqLabel.setText(_translate("preferencesDialog", "HS Freq"))
        self.lsFreqLabel.setText(_translate("preferencesDialog", "LS Freq"))
        self.bmlpfLabel.setText(_translate("preferencesDialog", "BM LPF"))
        self.peakQLabel.setText(_translate("preferencesDialog", "Peak Q"))
        self.lsQLabel.setText(_translate("preferencesDialog", "LS Q"))
        self.hsQLabel.setText(_translate("preferencesDialog", "HS Q"))
        self.bmlpfFreq.setSuffix(_translate("preferencesDialog", " Hz"))
        self.peakFreqLabel.setText(_translate("preferencesDialog", "Peak Freq"))
        self.toolBox.setItemText(self.toolBox.indexOf(self.filterPage), _translate("preferencesDialog", "Filter Defaults"))
        self.checkForBetaUpdates.setText(_translate("preferencesDialog", "Include Beta Versions?"))
        self.checkForUpdates.setText(_translate("preferencesDialog", "Check for Updates on startup?"))
        self.toolBox.setItemText(self.toolBox.indexOf(self.systemPage), _translate("preferencesDialog", "System"))
        self.beqDirectoryPicker.setText(_translate("preferencesDialog", "..."))
        self.beqDirectoryLabel.setText(_translate("preferencesDialog", "Directory"))
        self.toolBox.setItemText(self.toolBox.indexOf(self.beqPage), _translate("preferencesDialog", "BEQ"))
